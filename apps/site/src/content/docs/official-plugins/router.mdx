...
By default, `createRouterPlugin()` uses `scroll: "restore"`:

- **Push / replace** (new in-app navigation): scrolls `window` to the top.
- **Back / forward**: restores the saved scroll position for that pathname + query (`sessionStorage`).

```ts
// Legacy behavior (scroll unchanged on navigation)
const router = createRouterPlugin({ scroll: false });
// or per mount:
jd.router(routes, { scroll: false });

// Skip scroll-to-top for one link or navigate() call
jd.routerLink({ href: "/feed", preventScrollReset: true }, ["Load more"]);
...
function createRouterPlugin(options?: {
  mode?: "browser" | "hash";
  basename?: string;
  /** Default: `"restore"`. Use `false` to leave scroll unchanged. */
  scroll?: false | "restore";
}): JDPlugin<{
  router: (
...
```ts
jd.router(routes);
jd.router(routes, { basename: "/app", scroll: false });
```

`mountOptions.scroll` overrides the plugin default (`"restore"` or `false`). See [Scroll restoration](#scroll-restoration).
...
| `RouterMode`, `RouterMountOptions`, `NavigateOptions`, `ScrollBehavior` | Router configuration types |
