...
| `JDElementNamespace` | `"html" \| "svg" \| "mathml"` — optional 4th argument to [`createElement`](/docs/core-api/create-element) when disambiguating shared tag names |
| `JD_NAMESPACES` | Runtime URI map (`html`, `svg`, `mathml`) matching the DOM namespaces |
| `elementIsSvgOrMathML` | Predicate on `Element.namespaceURI` — useful instead of tag-name checks for HTML/SVG overlaps (`a`, `script`, …) |
| `JDRef<T>` | Ref object for a given tag type |
| `JDSvgDomKey<T>` | Name of the `DOM` method for an SVG tag (`svg`, or `svg` + `Capitalize` for other tags) |
| `JDom` | Type of the default `DOM` export object |

### Configured app DOM (`typeof jd`)

When you use [`withPlugins`](/docs/plugin-api/with-plugins) in a single app module (see **[App setup (jd.config)](/docs/jd-config)**), export a type alias so helpers stay aligned with your plugins:
...
New Vite + TypeScript projects can start from **[`create-just-dom`](/docs/jd-config#start-a-new-project)**.
