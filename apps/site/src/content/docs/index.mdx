...
Bootstrap **Vite + just-dom** (with `jd.config` and optional router) in one command:

```bash
npm create just-dom@latest
```

Details, flags (`--js`, `--no-router`, …), and monorepo usage: **[Installation](/docs/installation#new-vite-project-cli)** and **[App setup](/docs/jd-config#start-a-new-project)**.
