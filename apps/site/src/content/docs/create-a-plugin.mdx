...
In real apps, prefer registering plugins in one place—see **[App setup (jd.config)](/docs/jd-config)**—so every file imports the same `jd` instead of repeating `withPlugins`.
...
Application authors usually register published plugins once in **[App setup (jd.config)](/docs/jd-config)**. New Vite + TypeScript projects can start from **[`create-just-dom`](/docs/jd-config#start-a-new-project)**.
