...
| ------------------- | ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@just-dom/router`  | `createRouterPlugin()` in `jd.config`; **`defineRoutes([...])`** in `main` (o moduli che non importano `jd.config`) | Usare **`jd.routerLink`** per navigazione SPA (l’`a` HTML resta per link generici). Default **`scroll: "restore"`** (top su push/replace, ripristino su back/forward); opt-out `scroll: false`; **`preventScrollReset`** su link/`navigate`. Modalità **`hash`** utile senza fallback SPA lato server. Route: `path`, `index`, `layout`, `element`, `children`; **`path: "*"`** di solito ultimo tra fratelli. Doc: [router](https://just-dom.vercel.app/docs/official-plugins/router). Peer: `just-dom`. |
