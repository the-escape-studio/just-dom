...
2. Whether to install **`@just-dom/router`** (default: yes)

Non-interactive / CI: pass **`--yes`** to skip prompts (defaults: TypeScript + try router), or set each option explicitly:

| Flag | Meaning |
|------|---------|
| `--ts`, `--typescript` | Vite `vanilla-ts` template |
| `--js`, `--javascript` | Vite `vanilla` template (`.js` entry + `jd.config.js`) |
| `--router` | Install `just-dom` + `@just-dom/router` and add a starter route table |
...
The tool always invokes **`npm create vite`** for compatibility. If **`@just-dom/router` is not on npm** yet, the install step falls back to **`just-dom` only** and generates a **no-router** starter even when you asked for the router (check the generated `README.md`).
