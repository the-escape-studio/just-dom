...
For a larger app, you usually call `withPlugins` **once** in a dedicated module (conventionally [`jd.config`](/docs/jd-config)) and import the result as `jd` from your entry and components. To generate that layout in a new **Vite** app, use **[`create-just-dom`](/docs/jd-config#start-a-new-project)**.
